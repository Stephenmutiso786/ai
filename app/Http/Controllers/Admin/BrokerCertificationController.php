...
class BrokerCertificationController extends Controller {
    public function __construct(){ $this->middleware('can:manage-broker-certification'); }
    ...
    public function run(Request $request,BrokerAccount $account,BrokerCertificationService $service){ $service->certify($account,$request->user()->id); return back()->with('status','Broker certification completed.'); }
