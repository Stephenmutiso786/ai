...
class AiSignalClient
{
    public function liveSignal(AiModel $model, string $symbol, string $timeframe): array
    {
        ...
        if (! $url || ! $token) throw new \RuntimeException('AI service URL/token are not configured.');
        ...
        foreach ($providerConfig as $key => $value) {
            if ($value === null || $value === '') throw new \RuntimeException('Required market-data credential is missing from Super Admin Settings: '.$key);
        }
